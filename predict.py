...
The imput test dataset should be the integration of Mol2vec features for the drugs, ProtVec, Bionoi-AE and Graph2vec features for the proteins
...
def load_test_list(path):
    ...
    with open(path + 'test_list.pkl', 'rb') as f_test:
        test_list = pickle.load(f_test)

    print(len(test_list))

    with open(path + 'test_label.pickle', 'rb') as f_label:
        labels = pickle.load(f_label)

    return test_list, labels
...
def mlp_test(data_path, model_path, output_path, stage):

    in_path = 'test_list/'
    batch_size = 1000
    n_classes = 2
    feature_path = 'feature_list/'
    with open(feature_path + 'permu_feature_importance.json') as json_file:
    ...
    new_test_list, labels = load_test_list(in_path)
    ...
    if os.path.exists(output_path + str(stage) + '_roc_auc.pkl'):
        os.remove(output_path + str(stage) + '_roc_auc.pkl')
    if os.path.exists(output_path + str(stage) + "_fpr.pkl"):
        os.remove(output_path + str(stage) + "_fpr.pkl")
    if os.path.exists(output_path + str(stage) + "_tpr.pkl"):
        os.remove(output_path + str(stage) + "_tpr.pkl")
    if os.path.exists(output_path + str(stage) + "_thresh.pkl"):
        os.remove(output_path + str(stage) + "_thresh.pkl")
    # print(roc_auc)
    f1 = open(output_path + str(stage) + '_roc_auc.pkl', "wb")
    pickle.dump(roc_auc, f1)
    f1.close()
    f2 = open(output_path + str(stage) + "_fpr.pkl", "wb")
    pickle.dump(fpr, f2)
    f2.close()
    f3 = open(output_path + str(stage) + "_tpr.pkl", "wb")
    pickle.dump(tpr, f3)
    f3.close()
    f4 = open(output_path + str(stage) + "_thresh.pkl", "wb")
...
if __name__ == "__main__":
    ...
    for stage in stage_list:
        model_path = parse.model + str(stage) + '_mlp_supertarget_cluster.pt'
        print(model_path)
